...
def rand_brightness(x, aug_prob=0.5):
    b = Bernoulli(torch.Tensor[aug_prob])
...
def rand_saturation(x, aug_prob=0.5):
    ...
    x = x*keep_vec + ((x - x_mean) * (torch.rand(x.size(0), 1, 1, 1, dtype=x.dtype, device=x.device) * 2) + x_mean)*aug_vec
    return x
...
def rand_contrast(x, aug_prob=0.5):
    ...
    x = x*keep_vec + ((x - x_mean) * (torch.rand(x.size(0), 1, 1, 1, dtype=x.dtype, device=x.device) + 0.5) + x_mean)*aug_vec
    return x
...
def rand_translation(x, ratio=0.125, aug_prob=0.5):
    ...
    x = x*keep_vec + (x_pad.permute(0, 2, 3, 1).contiguous()[grid_batch, grid_x, grid_y].permute(0, 3, 1, 2))*aug_vec
    return x
